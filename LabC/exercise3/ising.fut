...
def random_grid (seed: i32) (h: i64) (w: i64)
    ...
    let rng            = rng_engine.rng_from_seed [rng]
    let rng_arr        = rng_engine.split_rng (h * w) rng
    let rand_grid_flat = 
      map (\r -> let (rng', spin) = rand_i8.rand (0i8, 1i8) r
           in  (rng', spin * 2 - 1)) rng_arr
    in (unflatten h w rand_grid_flat.0
      , unflatten h w rand_grid_flat.1)
    

-- Compute $\Delta_e$ for each spin in the grid, using wraparound at
-- the edges. delta = 2c(u + d + l + r)
def deltas [h][w] (spins: [h][w]spin): [h][w]i8 =
  let neigh_u = rotate (-1) spins
  let neigh_d = rotate 1 spins
  let neigh_l = map (rotate -1) spins
  let neigh_r = map (rotate 1) spins
  let delta c u d l r = 2 * c * (u + d + l + r)
  in (map5 delta <-< map5 delta)
       spins neigh_u neigh_d neigh_l neigh_r

-- The sum of all deltas of a grid.  The result is a measure of how
-- ordered the grid is.
def delta_sum [h][w] (spins: [w][h]spin): i32 =
  map i32.sum (deltas sum) |> i32.sum
